...
void main()
{
    ...
    vec3 sunDir = normalize(rotateX(vec3(0, 0.1, 1.0), time * 0.005)); //sun pos dependent on time
    ...
    if (moonAngle < moonSize) {
        tempColor = moonColor;
    } else if (sunUV.y < 0.4 || sunDir < -0.9) {
        ...
        tempColor = mix(uvToSunset(uv.y), uvToDusk(uv.y), a);
